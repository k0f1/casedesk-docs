...
docker run -d \
  ...
  -e OPENAI_API_BASE_URL=https://getcasedesk.com/v1 \
...
If your team already runs a shared Open WebUI instance, add your CaseDesk account as a connection.

**In Open WebUI:**

1. Go to **Settings > Connections**
2. Under **OpenAI API**, click the **+** icon to add a new connection
3. Set the Base URL to `https://getcasedesk.com/v1`
4. Set the API Key to your production API key (`cd_live_...`)
5. Click **Save**

Models approved for your active connection will appear in the model selector.
...
| Field | Value |
| --- | --- |
| Base URL | `https://getcasedesk.com/v1` |
...
Any tool that accepts a custom `base_url` and `api_key` can use CaseDesk once a supported model is available on an approved connection.
